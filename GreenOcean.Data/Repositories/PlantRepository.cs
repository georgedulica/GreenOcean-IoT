...
public class PlantRepository : IPlantRepository
{
    ...
    {
        var existingPlant = await _dataContext.Plants.AnyAsync(p => string.Equals(p.Name, plant.Name) && string.Equals(p.Soil, plant.Soil) && string.Equals(plant.Type, p.Type) && p.Height == plant.Height &&
            p.MositureLevel == plant.MositureLevel && p.Humidity == plant.Humidity && p.Temperature == plant.Temperature && p.GreenhouseId == plant.GreenhouseId);
